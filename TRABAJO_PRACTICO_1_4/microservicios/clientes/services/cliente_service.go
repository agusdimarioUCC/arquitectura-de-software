...
type ClienteService struct {
	Repositorio repositories.ClientesRepo
}

// NuevoClienteService cablea el servicio con su repositorio.
func NuevoClienteService(repositorio repositories.ClientesRepo) ClienteService {
	return ClienteService{Repositorio: repositorio}
}

// Crear valida y da de alta un cliente. El email es opcional.
func (servicio ClienteService) Crear(nombre, email string) (models.Cliente, error) {
	if strings.TrimSpace(nombre) == "" {
		return models.Cliente{}, fmt.Errorf("%w: el nombre es obligatorio", ErrValidacion)
	}
	return servicio.Repositorio.Crear(models.Cliente{Nombre: nombre, Email: email})
}

// BuscarPorID recupera un cliente por su identificador.
func (servicio ClienteService) BuscarPorID(identificador string) (models.Cliente, error) {
	if strings.TrimSpace(identificador) == "" {
		return models.Cliente{}, fmt.Errorf("%w: el id es obligatorio", ErrValidacion)
	}
	return servicio.Repositorio.BuscarPorID(identificador)
}
